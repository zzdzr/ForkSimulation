...
class Fork:
    def __init__(self, pos:int, attrs: Dict[str, bool] = None) -> None:
        """
        A fork has two important attributes: position (pos) and a dictionary of custom attributes (attrs).
        """
        self.pos = pos
    ...
    def move(self, termination_sites: np.ndarray, barrier_sites: np.ndarray, N: int) -> str:
        ...
        # print(f"[Move] From {self.pos} → {new_pos}, Termination sites: {termination_sites}")
...
class RepFactory:
    def __init__(self, left_fork: Fork, right_fork: Fork) -> None:
        """
        The Replication factory class represents a replication unit with two forks: left and right.
        """
...
class SingleMoleculeSimulator:
    ...
    def simulate_one_factory(self, sim_id: int) -> Dict:
        ...
        print(f"[Sim {sim_id}] Init pos: {factory.left_fork.pos}, {factory.right_fork.pos}, Term: {termination_sites}")
        while factory.coupled_state != 0:
            ...
            for fork in [factory.left_fork, factory.right_fork]:
                ...
                # print(f"FORK status is {status}, FACTORY coupled state is {factory.coupled_state}")
        ...
        return {
            "sim_id": sim_id,
            "coupled_states": np.array(coupled_states_data),
            "trajectory_path": np.array(path_data)
        }
    ...
    def run_simulations(self, num_simulations: int, output_file: str) -> None:
        for sim_id in range(num_simulations):
            ...
            result = self.simulate_one_factory(sim_id)
            self.trajectories_data.append(result)

        self.save_to_hdf5(output_file)
  
    def save_to_hdf5(self, output_file: str) -> None:
        """
        Save simulation data to HDF5 in a compatible format.
        
        Parameters:
        - file: Output HDF5 file name.
        - trajectories_data: List of dictionaries containing simulation data.
        ...
        with h5py.File(output_file, 'w') as myfile:
            ...
            # total length: sum the number of positions for all simulations
            total_length = np.nansum(trajectory_lengths)

            # Create a unified positions dataset
            ...
            for sim_idx, sim in enumerate(self.trajectories_data):
                ...
                # reshape current positions for hdf5
                cur_positions = cur_positions.reshape((length, 1, 2))

                # Fill the dataset from start_idx to start_idx + length
                dset[start_idx:start_idx + length, :, :] = cur_positions

                # Update start index for the next simulation segment
                start_idx += length

            # Check that all data has been filled correctly
            assert start_idx == total_length, "Data filling mismatch: start_idx does not match total length."
...
if __name__ == '__main__':
    ...
    for p in [0.1,0.3,0.5,0.7,0.9]: #
        ...
        output_name = f"decouplep_{p}_alpha_{args['alpha']}_add_noise_500_times"
        run_folder = f"./{output_name}"
        os.system(f"mkdir -p {run_folder}")
        file_1d = f"{run_folder}/01_simulations.h5"
        simulator.run_simulations(500, file_1d) # num_stimulation, output_file

        folder_3d = f"{run_folder}/3D_results"

        # run 3D
        os.system(f"mkdir -p {folder_3d}")
        command = 'python ./02_simulate.py --folder {} --myfile {}'.format(folder_3d, file_1d)
        os.system(command)
        
        command = 'python ./03_dynamic_plot.py --base_dir {}'.format(folder_3d)
        os.system(command)
